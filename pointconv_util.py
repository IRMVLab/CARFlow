...
def sample_and_group(npoint, radius, nsample, xyz, xyz_raw, label, points, knn=True, use_xyz=True):
    '''
    Input:
        npoint: int32
        radius: float32
    ...
    if npoint == 2048:
        ...
        new_points_raw, _, _, _ = torch.split(points, 2048, 1)  # (batch_size, 2048, c)
        ...
        new_points_raw = index_points_gather(points, sample_idx)  # (batch_size, npoint, c)
    ...
    if xyz_raw is not None:
        return grouped_xyz, new_xyz, new_points_raw, new_label, new_points, new_xyz_raw
    else:
        return grouped_xyz, new_xyz, new_points_raw, new_label, new_points  # (batch_size, npoint, 3) (batch_size, npoint, 3) (batch_size, npoint, nample, 3+c)


class PointNetSaModule(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channels, mlp, mlp2, group_all, is_training, bn_decay, bn=True,
        ...
        self.in_channels = 2 * mlp[-1] + in_channels
        if mlp2 is not None:
            for i, num_out_channel in enumerate(mlp2):
                self.mlp2_convs.append(Conv2d(self.in_channels, num_out_channel, [1, 1], stride=[1, 1], bn=bn))
                self.in_channels = num_out_channel

        self.xyz_encoding = Conv2d(10, mlp[-1], [1, 1], stride=[1, 1], bn=True)
    ...
    def forward(self, xyz, xyz_raw, label, points):
        ...
        if xyz_raw is not None:
            ...
            # sample npoint points in xyz and then find their knn points in xyz
            grouped_xyz, new_xyz, new_points_raw, new_label, new_points, new_xyz_raw = sample_and_group(self.npoint,
                                                                                                        self.radius,
                                                                                                        self.nsample,
                                                                                                        xyz,
                                                                                                        xyz_raw, label,
            ...
            grouped_xyz, new_xyz, new_points_raw, new_label, new_points = sample_and_group(self.npoint, self.radius,
        ...
        new_xyz_expanded = (torch.unsqueeze(new_xyz, 2)).repeat([1, 1, self.nsample, 1])  # B,npoint,nsample,3
        new_points_raw_expanded = (torch.unsqueeze(new_points_raw, 2)).repeat(
            [1, 1, self.nsample, 1])  # B,npoint,nsample,c

        new_xyz_diff = grouped_xyz - new_xyz_expanded

        new_xyz_euc_diff = torch.sqrt(torch.sum(torch.mul(new_xyz_diff, new_xyz_diff), dim=-1,
                                                keepdim=True) + 1e-20)  # B,npoint,nsample,1

        new_xyz_diff_concat = torch.cat([new_xyz_expanded, grouped_xyz, new_xyz_diff, new_xyz_euc_diff],
                                        dim=3)  # batch_size, npoints, nsample,10

        new_xyz_encoding = self.xyz_encoding(new_xyz_diff_concat)  # batch_size, npoints, nsample,mlp[-1]
        new_points_cat = torch.cat([new_points, new_xyz_encoding, new_points_raw_expanded],
                                   dim=-1)  # batch_size, npoints, nsample, 2*mlp[-1]+c

        if self.mlp2 is not None:
            for i, conv in enumerate(self.mlp2_convs):
                new_points_cat = conv(new_points_cat)  # batch_size, npoints, nsample,mlp2[-1]

        WQ = F.softmax(new_points_cat, dim=2)
        new_points = WQ * new_points
        # mlp[-1] = mlp2[-1]

        new_points = torch.sum(new_points, dim=2, keepdim=True)  # batch_size, npoint, 1, mlp2[-1]
...
class CostVolume(nn.Module):
    def __init__(self, radius, nsample, nsample_q, in_channels, mlp1, mlp2, is_training, bn_decay, bn=True,
                 pooling='max', knn=True, corr_func='elementwise_product'):
...
class SetUpconvModule(nn.Module):
    def __init__(self, nsample, in_channels, mlp, mlp2, is_training, bn_decay=None, bn=True, pooling='max', radius=None,
                 knn=True):
